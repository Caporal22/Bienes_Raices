...
function servidor() {
    browserSync.init({
        proxy: "localhost:8000",  // El host que voy a utilizar
        notify: false,            // Molestias en las notificaciones
        open: true                // Abrir el navegador automaticamente
    });

    watch(paths.scss, series(css, recargar));
    watch(paths.js, series(javascript, recargar));
    watch(paths.imagenes, series(imagenes, versionWebp, recargar));
}
